...
**Purpose:** Create the small, stable language that every source uses to describe one completed day of metrics, then store each source/day result safely.

**Why now:** Every later adapter and report needs the same meaning for a date, a status, and a metric. Defining that contract first prevents PostHog, Fly, and Shopify from each inventing incompatible shapes.

**How:** Validate snapshots at the boundary, write them atomically to a source/date location, and make completed snapshots immutable. A retry can improve an incomplete result, but it cannot silently rewrite known-good historical evidence.

> **DDIA lens — data models and derived data:** A data model is the contract through which the rest of the system understands facts. These snapshots are *derived data*: reproducible summaries imported from external systems, not the source systems of record. Their source/date key and immutable-complete rule give later weekly reviews a stable historical input.
...
**Purpose:** Establish one narrow boundary between Buffr's deterministic core and unreliable external services.

**Why now:** The adapters need HTTP access and credentials, but the storage and summary code should never know how a provider authenticates or how an HTTP failure is represented.

**How:** Put network requests, configuration reading, and error classification behind typed interfaces. Return small, categorized failures instead of leaking provider responses or secret values across the boundary.

> **DDIA lens — reliability through explicit failure handling:** A network call can fail even when the code is correct: credentials expire, rate limits apply, and connections break. Treating these as ordinary, named outcomes lets collection continue for the other sources and lets the final review state its limitations honestly.
...
**Purpose:** Turn MerchGrid's four privacy-safe product events into daily aggregate usage signals for Buffr.

**Why now:** This is the first real adapter and proves the contract using data already emitted by the live product: app opens and scan lifecycle outcomes.

**How:** Ask PostHog for counts in one completed UTC window, map only the approved totals into a snapshot, and calculate a completion rate only when there were scan starts. Persist totals, never raw events, persons, shop identities, or query responses.

> **DDIA lens — batch processing and aggregation:** This is a small bounded batch job: it asks for a closed time window, reduces many events into a few numbers, and stores the result. Aggregation makes the weekly review cheaper, safer, and easier to reason about than repeatedly querying raw event history.
...
**Purpose:** Bring Shopify's business aggregates—merchant activity, installs, uninstalls, and earnings—into the same daily snapshot model without guessing at an unsupported API.

**Why now:** These metrics complete the business picture, but the current trustworthy access path is an exported aggregate report rather than a verified Partner API integration.

**How:** Import only the chosen date and the five approved aggregate columns from a local CSV. Reject missing, duplicate, or malformed values, and label successful rows as a manual import so the weekly review retains provenance.

> **DDIA lens — provenance and data lineage:** A useful number carries its origin and collection method. The `manual_import` note records lineage: this snapshot came from a Partner export, not an automated live query. That context matters when judging freshness and confidence.
...
**Purpose:** Combine independently collected source snapshots into honest daily and weekly artifacts that Buffr's workflow engine can read.

**Why now:** Individual adapters are useful evidence, but a review needs one deterministic place to coordinate them, calculate comparisons, and state gaps.

**How:** Run adapters in a stable order, isolate each failure, save every result, then build summaries only from complete data. The weekly review compares two exact seven-day windows and leaves a value absent when either side is unavailable.

> **DDIA lens — fault isolation and materialized views:** Each source is an independent dependency, so one failure must not prevent the others from producing evidence. The summaries are *materialized views*: precomputed, deterministic interpretations of stored snapshots rather than a new source of truth.
...
**Purpose:** Make collection and review generation explicit, repeatable operations that a person or an external scheduler can run and verify.

**Why now:** The core should be proven end-to-end before it is automated in a hosted environment. Explicit commands create that operational seam without prematurely choosing a scheduler or deployment platform.

**How:** Expose one command for a completed daily window and one for two closed weekly windows. Test them with fakes, print only artifact paths and statuses, and defer scheduling until the execution host and secret store are chosen.

> **DDIA lens — idempotence and operational reproducibility:** An operation is idempotent when retrying it reaches the same correct state instead of duplicating data. Date-addressed snapshots and explicit commands mean an interrupted collection can be rerun safely and its result can be reproduced during debugging.
