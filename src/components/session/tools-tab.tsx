...
import { IconSearch, SourceIcon, sourceColor } from "@/components/icons";
...
export function ToolsTab() {
  ...
  return (
    <div>
      ...
      <div className="tools-tab__defaults">
        <div className="tools-tab__defaults-label">
            ...
            return (
              <label key={s} className="tools-tab__defaults-option">
                <input
                  ...
                  className="tools-tab__defaults-checkbox"
                />
                <span
                  className="tools-tab__defaults-source"
                  style={{ color: isConnected ? sourceColor(s) : "#555" }}
                >
                  <SourceIcon source={s} size={14} />
                  <span>{s}</span>
                </span>
                {!isConnected && <span className="tools-tab__defaults-note">(not configured)</span>}
              </label>
            );
          })}
        </div>
      ...
      {/* Integrations */}
      <div className="tools-tab__section-label tools-tab__section-label--mb">
        Integrations
      </div>

      {loading ? (
        <div className="tools-tab__skeleton-list">
          {[1, 2, 3].map((i) => (
            <div key={i} className="tools-tab__skeleton" />
          ))}
        </div>
      ) : (
        <div className="tools-tab__integration-grid">
          {integrations.map((integration) => (
            <div key={integration.id} className="tools-tab__integration">
              <span
                className="tools-tab__integration-icon"
                style={{ color: sourceColor(integration.id) }}
              >
                <SourceIcon source={integration.id} size={18} />
              </span>
              <div className="tools-tab__integration-body">
                <div className="tools-tab__integration-header">
                  <span className="tools-tab__integration-name">{integration.name}</span>
                  <Badge
                  ...
                  <span className="tools-tab__integration-tool-count">
                    {integration.tools.length} tool{integration.tools.length !== 1 ? "s" : ""}
                  </span>
                </div>
                <div className="tools-tab__integration-desc">{integration.description}</div>
              </div>
              <div className="tools-tab__integration-actions">
      ...
      {/* Tool Registry */}
      <div className="tools-tab__registry-header">
        <div className="tools-tab__section-label">
          Tool Registry ({filteredTools.length})
        </div>
        <div className="tools-tab__registry-controls">
          <div className="tools-tab__registry-search">
            <span className="tools-tab__registry-search-icon">
              <IconSearch size={12} />
            </span>
            <input
              value={toolQuery}
              ...
              className="tools-tab__registry-search-input"
            />
          </div>
          <div className="tools-tab__registry-filter">
            {["all", ...integrations.map((i) => i.id)].map((f) => (
              <button
                key={f}
                onClick={() => setToolFilter(f)}
                className={`tools-tab__registry-filter-btn ${
                  toolFilter === f
                    ? "tools-tab__registry-filter-btn--active"
                    : "tools-tab__registry-filter-btn--inactive"
                }`}
              >
                {f === "all" ? (
                  "All"
                ) : (
                  <span className="tools-tab__registry-filter-source" style={{ color: sourceColor(f) }}>
      ...
      <div className="tools-tab__registry">
        <div className="tools-tab__registry-head">
          <span>Tool</span>
          <span>Parameters</span>
          <span>Source</span>
        </div>
        <div className="tools-tab__registry-body">
          {filteredTools.map((tool) => (
            <div key={tool.name} className="tools-tab__registry-row">
              <div>
                <span className="tools-tab__registry-tool-name">{tool.name}</span>
                <span className="tools-tab__registry-tool-desc">{tool.description}</span>
              </div>
              <span className="tools-tab__registry-tool-params">
                {tool.inputSchema
          ...
          {filteredTools.length === 0 && (
            <div className="tools-tab__registry-empty">
