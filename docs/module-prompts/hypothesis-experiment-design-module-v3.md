...
- a **PROCEED TO HYPOTHESIS** decision, primary bottleneck, supporting evidence, competing explanation, and confidence from **M4 — Classification & Diagnosis**,
- current product and listing presentation from **M1 — Product Context / Observation** so the proposed revision is grounded in the actual listing,
- findings from **M3 — Domain Research** when a blocking knowledge gap was researched.

Do not reopen the accepted diagnosis unless the supplied inputs are internally inconsistent.

## Dependencies

- **M0 — Core Agent:** required shared reasoning and guardrails.
- **M4 — Classification & Diagnosis:** required diagnostic basis.
- **M1 — Product Context / Observation:** required current product/listing context for a specific revision.
- **M3 — Domain Research:** conditional/callable when a blocking knowledge gap prevents experiment design.
