...
```
  ...
  │  Adds: pgvector, local Gemma, single-device,             │
  │  consumes a toolkit as a library.                        │
  └───────────────────────────────┬───────────────────────────┘
                                  │  "tell me more"
...
> "It's buffr — a personal RAG agent that runs entirely on my laptop. I point it at my own markdown notes, it indexes them into Postgres with pgvector, and then I can ask it questions in natural language. A local Gemma model answers, grounded in the chunks it retrieves — nothing leaves the machine.
>
> The shape is the classic retrieve-augment-generate loop, but with two deliberate constraints. First, it's local-first: the embeddings come from a local nomic model, generation is local Gemma through Ollama, and the vectors and my relational data live in the same single Postgres instance. Second, I consume an agent toolkit — aptkit — as a library and never edit it; my job was the persistence layer, the pgvector adapter, the CLI, and the trajectory capture on top.
>
> The piece I'm proudest of is the storage boundary. I implemented the toolkit's VectorStore contract over pgvector, so the agent loop has no idea it's talking to Postgres instead of an in-memory store — same contract, swapped body. The thing I'm still watching is evaluation: I score retrieval with precision and recall, but I don't yet measure whether the answer is faithful to the chunks. That's the next thing I'd build."
...
> "buffr is a laptop RAG agent. I index my own notes into Postgres with pgvector, then ask questions and a local Gemma model answers from the retrieved chunks — fully local, no cloud. I built the persistence and CLI layer on top of an agent toolkit I consume as a library."
  ...
  You give the 90-second pitch.
        ...
        └─► IF THEY ASK "did you build the agent loop?"
              → Be exact: no, I consume it from aptkit. I
                ...
                the CLI, the trace sink. Chapter 8 trains
                this honesty.
```
...
- *"Tell me about a project."* → buffr: a laptop RAG agent, index my notes into pgvector, ask questions, local Gemma answers from retrieved chunks, nothing leaves the machine.
- *"Why local-first?"* → Privacy over my own notes, no per-query cost, and it forced the hard local-model problems.
- *"Did you build the agent loop?"* → No — I consume aptkit as a library. I built persistence, the pgvector adapter, the CLI, the trace sink.
