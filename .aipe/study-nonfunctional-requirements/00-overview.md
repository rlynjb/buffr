...
`session.ts` — the most complex code in the repo — has zero tests on its session methods (`ask()`, `analyze()`, `evalInvesting()`). A `try/catch` in the memory write deliberately eats errors so a memory failure doesn't lose the user's answer, but no test asserts that swallow is correct. Suite is now 32 tests / 7 files (up from 9/6) but most DB-gated tests still skip without `DATABASE_URL`; CI never provisions Postgres. One test file for `session.ts` with injected fakes + a CI pgvector service container are the next-action fixes.
